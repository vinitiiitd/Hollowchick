...
public class gameplay_s implements Screen {
    ...
    private List<pig> pigs = loadPigs("game_p.dat");
    ...
    private List<Bird> birds = loadbirds("gameState_bi.dat");
...
//        world = new World(new Vector2(0, -9.8f), true);
        ...
        // Create bird
        if(p1){pig1 = pigs.get(0);}
        pig1.getBody().setUserData("pig1");
        if(p2){pig2 = pigs.get(1);}
...
//        b3.setPosition(0.1f,1.5f);
        ...
        // Create blocks
        blocks = loadBlocks("gameState_bl.dat");
        if(b_1 = true){block1 = blocks.get(0);}
        block1.getBody().setUserData("b1");
        if(b_2 = true){block2 = blocks.get(1);}
        block2.getBody().setUserData("b2");
        if(b_3 = true){block3 = blocks.get(2);}
        block2.getBody().setUserData("b3");
        if(b_4 = true){block4 = blocks.get(3);}
        block4.getBody().setUserData("b4");
        if(b_5 = true){block5 = blocks.get(4);}
        block5.getBody().setUserData("b5");
        if(b_6 = true){block6 = blocks.get(5);}
        block6.getBody().setUserData("b6");
        if(b_7 = true){block7 =blocks.get(6);}
        block7.getBody().setUserData("b7");
        if(b_8 = true){block8 = blocks.get(7);}
        block8.getBody().setUserData("b8");
        if(b_9 = true){block9 = blocks.get(8);}
...
//
    ...
    public static List<Block> loadBlocks(String filename) {
        try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(filename))) {
            return (List<Block>) ois.readObject(); // Deserialize the list of blocks
        } catch (IOException | ClassNotFoundException e) {
            System.out.println("Error loading blocks from file: " + e.getMessage());
            return null;
        }
    }
    public static List<pig> loadPigs(String filename) {
        try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(filename))) {
            return (List<pig>) ois.readObject(); // Deserialize the list of pigs
        } catch (IOException | ClassNotFoundException e) {
            System.out.println("Error loading pigs from file: " + e.getMessage());
            return null;
        }
    }
    public static List<Bird> loadbirds(String filename) {
        try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(filename))) {
            return (List<Bird>) ois.readObject(); // Deserialize the list of pigs
        } catch (IOException | ClassNotFoundException e) {
            System.out.println("Error loading pigs from file: " + e.getMessage());
            return null;
        }
